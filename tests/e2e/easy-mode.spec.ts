...
test.describe("Style Atelier Sandbox E2E Tests - Easy Mode Workbench Modal", () => {
  ...
  test("should open Simple Workbench modal on card click in Easy Mode, adjust prompt, and inject successfully", async ({ page }) => {
    const screenshotsDir = path.join(__dirname, "../../tests/screenshots");
    console.log("Navigating to sandbox page for Easy Mode Workbench E2E test...");
    ...
    const modalTitle = spFrame.locator("h3:has-text('Simple Workbench'), h3:has-text('簡易 Workbench')");
    ...
    await expect(mjTextarea).toHaveValue("cute cat illustration, blue --ar 16:9", { timeout: 10000 });

    // 8. Close the modal by clicking 'Cancel'
    const cancelBtn = spFrame.locator("button:has-text('Cancel'), button:has-text('キャンセル')");
