...
test.describe("Style Atelier Sandbox E2E Tests - Prompt Injection @J-WB-EXPERT-03", () => {
  ...
  test("should render Midjourney mock and Sidepanel side-by-side and inject prompt", async ({
    page
  }) => {
    ...
    try {
      ...
      const injectButton = spFrame.locator(
      ...
      console.log(
        `Success screenshot saved to tests/screenshots/side-by-side-success.png`
      )
    } catch (error) {
  ...
  test("should correctly extract image prompts and image srefs from mock DOM", async ({
    page
  }) => {
  ...
  test("should correctly extract Job ID, prompt, and image source from pattern2.html sandbox variant", async ({
    page
  }) => {
