...
describe("CardThumbnail", () => {
  ...
  it("renders two thumbnails side-by-side when thumbnailImages has 2 images", () => {
    ...
    render(
      <CardThumbnail thumbnailImages={images} alt="Test Card" tier="Rare" />
    )
  ...
  it("renders category custom icon image when provided", () => {
    const category = {
      id: "custom",
      name: "Custom",
      iconUrl: "data:image/png;base64,icon_data"
    }
    render(
      <CardThumbnail
        imageUrl="https://example.com/one.png"
    ...
    const categoryImg = screen
      .getAllByRole("img")
      .find(
        (img) => img.getAttribute("src") === "data:image/png;base64,icon_data"
      )
