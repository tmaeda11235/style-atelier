...
- **PWA Core Technologies**: Web App Manifest (`manifest.json` for Standalone Shell), Service Worker (`sw.ts` with Cache API for offline caching), IndexedDB/OPFS (for client-side storage), Web Share API (for mobile-native sharing).
...
- **Local AI**: `@litert-lm/core` (LiteRT-LM) - For loading and executing the quantized Gemma-2 2B model client-side using WebGPU/Wasm. Currently used to parse natural language search queries into structured filters (rarity, category, color, keyword).
...
- **Mobile PWA OAuth2 Security & Domain Constraints**: モバイルPWAは拡張機能コンテキスト外（独立ドメイン）で動作するため、Google Drive OAuth2認証におけるリダイレクト制限が発生する。Standalone (A2HS) 起動時は外部遷移によるセッションロストを防ぐため、Google Identity Services (GIS) TokenClientによる **Popup flow** を強制し、`postMessage` 経由でバックグラウンドでのセッション中継を行う。Google Cloud Console上での認証設定（Authorized JavaScript origins に `https://tmaeda11235.github.io`、Redirect URIs に callback HTML パスを登録）の厳密な管理が必要。
- **LiteRT-LM & WebGPU Compatibility**: Running Gemma-2 2B locally requires WebGPU support in the browser for optimal performance. While fallbacks (Wasm) exist, WebGPU is highly recommended to keep inference times practical.
- **Model Storage & Download Overhead (Quota & Integrity)**: The quantized Gemma-2 2B model (~1GB) must be downloaded over the network on the first run and cached in the browser's Cache API or Origin Private File System (OPFS). The application must perform a pre-download storage check using `navigator.storage.estimate()` (requiring at least 1.5 GB free space) to avoid `QuotaExceededError`. It must also execute cache integrity validations at startup/load to detect and purge incomplete or corrupted cache files.
