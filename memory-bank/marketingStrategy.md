...
- **ステータス**: `🟢 施策実行中（計画・トリアージ段階）`
- **今後のロードマップ**:
  1. **QRコードおよびWebRTCを用いたローカルP2P直接転送機能**（または一時キャッシュサーバー経由の匿名同期）のプロトタイプ設計と実装（[Issue #1263](https://github.com/tmaeda11235/style-atelier/issues/1263)）。
  2. **モバイル上のWebLLM/LiteRT-LMローカルAI推論**の統合検証と、iOSの1.5GB OOM制限を回避する「ハイブリッド・フォールバック推論（モデルサイズ自動選択）」の構築（[Issue #1264](https://github.com/tmaeda11235/style-atelier/issues/1264)）。
