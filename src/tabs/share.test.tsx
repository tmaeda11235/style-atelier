...
describe("SharePage i18n and functionality", () => {
  const mockCard = {
    ...
    tier: "LEGENDARY",
    category: "Sci-Fi",
    promptSegments: [{ value: "/imagine prompt:" }, { value: "cyberpunk samurai --ar 16:9" }],
  };
  ...
  beforeEach(() => {
    ...
    vi.spyOn(console, "error").mockImplementation((...args) => {
      capturedErrors.push(args.map(arg => arg instanceof Error ? arg.message : String(arg)));
    });
  ...
  it("renders English strings when navigator.language is en", async () => {
    ...
    expect(screen.getByText(/Tip: You can paste the copied card image/)).toBeDefined();
  ...
  it("displays Japanese error when load fails and navigator.language is ja", async () => {
    setLanguageAndClipboard("ja-JP");
    vi.mocked(db.styleCards.get).mockResolvedValue(null); // style card not found

    render(<SharePage />);

    await waitFor(() => {
      expect(screen.getByText("カード読み込みエラー")).toBeDefined();
      expect(screen.getByText("データベース内にスタイルカードが見つかりません。")).toBeDefined();
  ...
  it("displays English error when load fails and navigator.language is en", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("Style card not found in database.")).toBeDefined();
  ...
  it("handles copy to clipboard and shows success message in Japanese", async () => {
    ...
    await waitFor(() => {
      if (capturedErrors.length > 0) {
        throw new Error("Captured Errors: " + capturedErrors.map(e => e.join(" | ")).join("\n"));
      }
      expect(mockClipboard.write).toHaveBeenCalled();
      expect(screen.getByText("カード画像をクリップボードにコピーしました！DiscordやX等に貼り付けられます。")).toBeDefined();
    });
  });
  ...
  it("handles copy to clipboard and shows success message in English", async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockClipboard.write).toHaveBeenCalled();
      expect(screen.getByText("Copied card image to clipboard! You can now paste it into Discord, X, etc.")).toBeDefined();
    });
  });
