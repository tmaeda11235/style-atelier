...
function getNextTier(currentTier: RarityTier): RarityTier | null {
  switch (currentTier) {
    case "Common":
      return "Rare"
    case "Rare":
...
function canEvolve(card: StyleCard): boolean {
...
async function evolveCard(cardId: string) {
...
async function createVariation(
  ...
  const mergedSegments = mergePromptSegments(
    parentCards.flatMap((p) => p.promptSegments)
  )
  const mergedParams = mergeParameters(parentCards, mainParent)
