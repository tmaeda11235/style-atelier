...
import { ParameterArrayEditor } from "./ParameterArrayEditor";
...
describe("ParameterArrayEditor", () => {
  ...
  it("calls onChange with updated list when adding a new value via the Plus button", () => {
    ...
    const plusButton = screen.getAllByRole("button").find(btn => btn.querySelector("svg.lucide-plus"))!;
  ...
  it("calls onChange with filtered list (or undefined if empty) when deleting a value", () => {
    ...
    const xButtons = screen.getAllByRole("button").filter(btn => btn.querySelector("svg.lucide-x"));
