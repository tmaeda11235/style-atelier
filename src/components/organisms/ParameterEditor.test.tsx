...
describe("ParameterEditor", () => {
  ...
  it("should add a parameter value when clicking the Add button", () => {
    const mockOnChange = vi.fn();
    render(<ParameterEditor parameters={{ p: ["yes"] }} onChange={mockOnChange} />);
    ...
    const addButtons = screen.getAllByRole("button").filter(btn => btn.querySelector("svg.lucide-plus"));
  ...
  it("should remove a parameter value when clicking the remove button", () => {
    const mockOnChange = vi.fn();
    render(<ParameterEditor parameters={{ p: ["yes", "code1"] }} onChange={mockOnChange} />);
  ...
  it("should load autocomplete suggestions from existing cards in database", () => {
    ...
    expect(screen.getByText("https://example.com/existing-ip.png")).toBeDefined();

    fireEvent.focus(inputs[1]); // Style Reference
    expect(screen.getByText("https://example.com/existing-sref.png")).toBeDefined();

    const crefInput = screen.getByPlaceholderText("Add Character URL");
    fireEvent.focus(crefInput);
    expect(screen.getByText("https://example.com/existing-cref.png")).toBeDefined();
  });
});
