...
interface GodRaysAndBgCirclesProps {
  configColor: string
  newTier: RarityTier
  isFlipped: boolean
}

const GodRaysAndBgCircles: React.FC<GodRaysAndBgCirclesProps> = ({
  configColor,
  newTier,
  isFlipped
}) => (
...
const ModalHeader: React.FC<ModalHeaderProps> = ({ onClose, translation }) => (
  <>
    <button
      onClick={onClose}
    ...
    <div className="fixed inset-0 bg-black/20 dark:bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 animate-in fade-in duration-300">
      ...
      <div className="relative bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-sm w-full mx-4 shadow-2xl flex flex-col items-center overflow-hidden animate-in zoom-in-95 duration-500 z-20">
        <GodRaysAndBgCircles
          ...
          isFlipped={isFlipped}
        />
        <ModalHeader onClose={onClose} translation={translation} />
        ...
        <ModalFooter
          ...
          newTier={newTier}
          onClose={onClose}
          translationClose={translation.close}
        />
      </div>
    </div>
  )
