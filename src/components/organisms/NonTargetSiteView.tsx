...
export function NonTargetSiteView({
  onOpenMidjourney,
  isEmbedded = false
}: NonTargetSiteViewProps) {
  return (
    <div
      className={`w-full flex flex-col items-center justify-center p-6 text-slate-200 font-sans select-none overflow-hidden ${
        isEmbedded
          ? "h-[calc(100vh-140px)] min-h-[400px] rounded-2xl bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 border border-slate-800 shadow-xl"
          : "h-screen bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950"
      }`}>
