...
const ParameterDisplaySection: React.FC<ParameterDisplaySectionProps> = ({
  ...
  return (
    <div className="space-y-2 bg-slate-50/50 p-3 rounded-lg border border-slate-100 text-xs">
      {editedParams.ar && (
        <div>
          <span className="font-bold text-slate-500">{t.aspectRatio}</span>{" "}
          {editedParams.ar}
        </div>
      )}
      {editedParams.p && editedParams.p.length > 0 && (
        <div>
          <span className="font-bold text-slate-500">{t.personalization}</span>{" "}
          {editedParams.p.join(", ")}
        </div>
      )}
      {editedParams.imagePrompts && editedParams.imagePrompts.length > 0 && (
        <div>
          <span className="font-bold text-slate-500">{t.imagePrompts}</span>{" "}
          {editedParams.imagePrompts.join(", ")}
        </div>
      )}
      {editedParams.sref && editedParams.sref.length > 0 && (
        <div>
          <span className="font-bold text-slate-500">{t.styleReference}</span>{" "}
