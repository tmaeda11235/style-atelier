...
describe("ShareCardModal", () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal("ClipboardItem", vi.fn().mockImplementation(function(obj: any) {
      return obj
    }))
  })
  ...
  it("handles copy to clipboard success", async () => {
    ...
    expect(defaultProps.addLog).toHaveBeenCalledWith('Copied card "Neon Dragon" to clipboard.')
    expect(defaultProps.onClose).toHaveBeenCalled()
  })

  it("displays error message if copy to clipboard fails", async () => {
    const { renderCardToCanvas } = await import("../../lib/export-utils")
    vi.mocked(renderCardToCanvas).mockRejectedValue(new Error("Canvas render failed"))
    ...
    expect(screen.getByText("Failed to copy image to clipboard: Canvas render failed")).toBeDefined()
  })
  ...
  it("handles download success", async () => {
    ...
    expect(defaultProps.addLog).toHaveBeenCalledWith('Downloaded card "Neon Dragon" as PNG.')
    expect(defaultProps.onClose).toHaveBeenCalled()
  })

  it("displays error message if download fails", async () => {
    const { exportCardAsImage } = await import("../../lib/export-utils")
    vi.mocked(exportCardAsImage).mockRejectedValue(new Error("Download blocked"))
    ...
    expect(screen.getByText("Failed to download image: Download blocked")).toBeDefined()
  })
})
