...
import { WebDataExtractor } from "./WebDataExtractor"
...
describe("WebDataExtractor", () => {
  ...
  it("should extract simple prompt and jobId from standard anchor element structure", () => {
    ...
    expect(result?.fullCommand).toBe("A majestic golden lion, sunrise --ar 16:9 --sref 12345")
    expect(result?.imageUrl).toBe("https://cdn.midjourney.com/88888888-4444-4444-4444-121212121212/0_0.png")
  ...
  it("should extract image prompts, image srefs, and crefs from button elements and merge them into the prompt", () => {
    ...
    expect(result?.fullCommand).toBe("https://s.mj.run/image_prompt_url A cute anime girl holding a mug --ar 1:1 --sref 99999 https://s.mj.run/sref_url --cref https://s.mj.run/cref_url")
  ...
  it("should fall back to sibling anchor job links and use main image URL when dragging a reference image button", () => {
    ...
    `
    document.body.appendChild(container)
    const srefImg = document.getElementById("dragged-sref-img") as HTMLImageElement

    const result = extractor.extract(srefImg)

    expect(result).not.toBeNull()
    expect(result?.id).toBe("11111111-2222-3333-4444-555555555555")
    expect(result?.fullCommand).toBe("A scenic landscape, mountains --ar 16:9 --sref https://s.mj.run/sref_url_2")
    // imageUrl should be the main image URL from the sibling job link
    expect(result?.imageUrl).toBe("https://cdn.midjourney.com/11111111-2222-3333-4444-555555555555/0_0.png")
